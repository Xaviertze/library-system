...
/**
 * POST /api/books/return/:borrow_id
 * 归还图书
 */
router.post('/return/:borrow_id', authenticate, authorize('student', 'staff'), (req, res) => {
  const { borrow_id } = req.params;
  const userId = req.user.id;

  // 1. 检查借阅记录是否存在且属于该用户
  const record = db.prepare(`SELECT * FROM borrow_records WHERE id = ? AND user_id = ? AND status = 'active'`).get(borrow_id, userId);
  
  if (!record) {
    return res.status(404).json({ error: '未找到活跃的借阅记录' });
  }
  ...
  const returnTransaction = db.transaction(() => {
    ...
    db.prepare(`
      ...
      WHERE id = ?
    `).run(new Date().toISOString(), borrow_id);
...
router.get('/view/:id', authenticate, (req, res) => {
  ...
  // 设置 Content-Type 为 PDF，并设置为 inline 让浏览器尝试预览而非下载
...
/**
 * GET /api/books/annotations/:book_id
 * 获取用户的阅读进度和高亮标注
 */
router.get('/annotations/:book_id', authenticate, (req, res) => {
  const { book_id } = req.params;
  const userId = req.user.id;

  try {
    const data = db.prepare(`
      SELECT last_page, highlights 
    ...
    // 如果没有记录，返回初始默认值
    if (!data) {
      return res.json({ last_page: 1, highlights: [] });
    }

    // 数据库存的是字符串，需要解析成 JSON 数组返回给前端
    res.json({
    ...
    res.status(500).json({ error: '无法加载阅读进度' });
  }
});

/**
 * POST /api/books/annotations
 * 保存或更新阅读进度和高亮标注
 */
router.post('/annotations', authenticate, (req, res) => {
  const { book_id, last_page, highlights } = req.body;
  const userId = req.user.id;
  ...
  try {
    ...
    // 使用 SQLite 的 UPSERT 语法：如果记录存在则更新，不存在则插入
    db.prepare(`
      INSERT INTO user_annotations (id, user_id, book_id, last_page, highlights, updated_at)
      VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
      ON CONFLICT(user_id, book_id) DO UPDATE SET
        last_page = excluded.last_page,
        highlights = excluded.highlights,
        updated_at = CURRENT_TIMESTAMP
    `).run(uuidv4(), userId, book_id, last_page || 1, highlightsStr);

    res.json({ message: 'Progress saved successfully' });
  } catch (err) {
    console.error('保存标注失败:', err);
    res.status(500).json({ error: '无法保存阅读进度' });
  }
});
